...
* Mount a subpath of the MooseFS filesystem specified by the `REMOTE_PATH` environment variable ( `/docker/volumes` by default) to `/mnt/moosefs`. This is where the docker volumes will be stored. The `/mnt/moosefs` directory will be referred to as the "volume root" throughout this document.
...
```json
{
    "Name": "volume_name",
    "Opts": {
      "StorageClass": "storage_class_number_or_name"
    }
}
```
...
* Create sub-directory of volume root with the given `Name`. For example, `/mnt/moosefs/volume_name`.
* Use `mfssetsclass` to set the replication goal for that Docker Volume to the value specified in the `Opts` ( if specified ).
...
* Delete the directory in the volume root with the given `Name`. For example, `/mnt/moosefs/volume_name`.
...
* Create a directory outside of the MooseFS root mountpoint using the given `Name`, such as `/mnt/docker-volumes/volume_name`.
* Mount the subpath of the MooseFS filesystem ( for example, `/docker/volumes/volume_name` ) to the newly created mountpoint.
...
* Get a list of the directories in the volume root: `/mnt/moosefs/`.
